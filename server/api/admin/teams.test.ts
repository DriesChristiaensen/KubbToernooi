...
function createMockEvent(overrides: any = {}) {
  return { _url: 'http://localhost/api/admin/teams', context: {}, ...overrides } as any
}
...
describe('GET /api/admin/teams', () => {
  ...
  it('returns 404 when no tournament exists', async () => {
    ...
    await expect(getTeamsHandler(createMockEvent())).rejects.toThrow('No tournament found')
  })
...
describe('POST /api/admin/teams', () => {
  ...
  it('returns 400 when name is missing', async () => {
    ...
    await expect(createTeamHandler(createMockEvent())).rejects.toThrow('Missing name field')
  })
  ...
  it('returns 409 when team name already exists', async () => {
    ...
    await expect(createTeamHandler(createMockEvent())).rejects.toThrow('Duplicate team name')
  })
  ...
  it('creates a team successfully', async () => {
    ...
    mockTeamCreate.mockResolvedValue({ id: 1, name: 'Team A', tournamentId: 1 })
...
describe('PUT /api/admin/teams/:id', () => {
  ...
  it('returns 400 for invalid ID', async () => {
    ...
    await expect(updateTeamHandler(createMockEvent())).rejects.toThrow('Invalid team ID')
  })
  ...
  it('returns 400 when name is missing', async () => {
    ...
    await expect(updateTeamHandler(createMockEvent())).rejects.toThrow('Missing name field')
  })
  ...
  it('returns 409 when new name is a duplicate', async () => {
    ...
    await expect(updateTeamHandler(createMockEvent())).rejects.toThrow('Duplicate team name')
  })
  ...
  it('updates team name successfully', async () => {
    ...
    vi.mocked(readBody).mockResolvedValue({ name: 'Team B' })
    mockTeamFindFirst.mockResolvedValue(null)
    mockTeamUpdate.mockResolvedValue({ id: 1, name: 'Team B', tournamentId: 1 })
...
describe('DELETE /api/admin/teams/:id', () => {
  ...
  it('returns 400 for invalid ID', async () => {
    ...
    await expect(deleteTeamHandler(createMockEvent())).rejects.toThrow('Invalid team ID')
  })
