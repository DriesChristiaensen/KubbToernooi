...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    throw createApiError({ error: "Titel is verplicht", code: "rule_group_title_empty", reason: "Empty title" });
  }
  ...
  const group = await prisma.ruleGroup.update({ where: { id }, data: { title: body.title } });
  logRequest(event, "success", `RuleGroup updated: ${id}`);
  return group;
});
